...
$hide = '';
$cek_val = 'on';
if(isset($json['switch_galery2'])){
    if($json['switch_galery2'] == 'off'){
      $cek = '';
      $hide = 'd-none';
      $iframe = '../../error';
      $cek_val = 'off';
...
<!-- proges box -->
    ...
    <input type="hidden" id="count" name="count" value="<?= $img_count; ?>">
                  ...
                  <div class="d-flex align-items-md-start row">
                    ...
                    <div class="col-sm-6">
                      <div class="card-body">
                        ...
                        <h4 class="text-primary">Galery Tambahan</h4>
                      </div>
                        <hr />
                        <div class="mb-2">
                        <strong class="sw-off text-hide">off</strong>
                        <label class="heart-switch">
                            <input name="switch_galery2" type="checkbox" id="sw-input" value="<?= $cek_val; ?>" <?= $cek; ?>>
                        ...
                        <div id="data-galery" class="input-box text-nowrap <?= $hide; ?>">
                          <div class="mb-3 row">
                                  <!-- Uploaded -->
          <?php if(!empty($imgs)) :?>
            <?php foreach($imgs as $img): ?>
              <?php $str = substr($img,0,-4) ?>
                          <div id="<?= $str; ?>" class="col-sm-6 imgUpload">
                            <div style="background-image: url('<?= $galery_path.$img; ?>');" class="imageUploaded">
                            </div>
                            <label onclick="deleteImg('<?= $str; ?>','<?= $img; ?>')" class="btn btn-danger btn-square">Delete</label>
                          </div>
            <?php endforeach; ?>
          <?php endif; ?>
                                  <!-- Uploaded -->
                                  <!-- upload box -->
          <?php if(count($imgs) < 10) :?>
                          <div class="col-sm-6 imgUp">
                            <div class="imagePreview"></div>
                                <label class="btn btn-primary btn-square">Upload
                                    <input type="file" class="uploadFile img" value="Upload Photo" style="width: 0px;height: 0px;overflow: hidden;" accept="image/jpeg">
                            </label>
                            <i class="bx bx-x bx-md del"></i>
                          </div><!-- col-2 -->
                          <div class="imgAdd">
                            <i style="margin-left: -6.4px;" class="bx bx-plus-medical"></i>
                          </div>
          <?php endif; ?> 
                                    <!-- upload box -->
                          </div>
                          <div class="mb-2 row">
                            <button class="btn btn-primary w-px-100 " type="button" name="submit" onclick="apply('<?= $user; ?>','galery2')">Apply</button>
                          </div>
                        </div>
                          <div class="hide-btn mb-2 row d-none">
                            <button class="btn btn-warning w-px-100" type="button" name="submit" onclick="apply('<?= $user; ?>','galery2')">Apply</button>
                          </div>                                                         
                      </div>
                    </div>
                      <!-- input -->
                    ...
                    <div class="col-sm-12">
                      <div class="card-body">
                        ...
                        <div class="col-6 col-md-3">
                              <a id="prev" href="galery.php?user=<?= $user; ?>" class="btn btn-gray" style="background-color: #bb818a;" name="previous"><< prev</a>                              
                            </div>
                            <div class="col-6 col-md-3">
                              <a id="next" href="rsvp.php?user=<?= $user; ?>" class="btn btn-gray" style="background-color: #bb818a;" name="next">Next >></a>                              
                            </div>
                        </div>
                      </div>
                    </div>
  ...
  >
    ...
    <script src="js/upload.js?dev=<?= $versi; ?>"></script>
