...
$data_reset = mysqli_real_escape_string($conn,$data['data_reset']);
...
// HEAD
if($page == 'head'){
  $iframe = "../../theme/$tema/preview.php?user=$user&page=head";
        // head
    $img_content = "https://ada-undangan.online/theme/$tema/images/rsvp/img1.jpg";
        // PATH IMAGE COVER
    $cover_path = "../../dari/$folder_name/imgs/cover/";
    $head_img = $cover_path.'img_content.jpg';
    
                    // resize cover image
    if(isset($_FILES['head_img']) && $_FILES['head_img']['type'] == 'image/jpeg'){
        $file = $head_img;
                if($_FILES["head_img"]['size'] > 290000){
                    move_uploaded_file($_FILES['head_img']['tmp_name'],$file);
    ...
    if(file_exists($head_img)){
      $img_content = "https://ada-undangan.online/dari/$folder_name/imgs/cover/img_content.jpg";
    }

    if(!isset($_FILES['head_img'])){
      $img_content = $_POST['img_content'];
    }
        $data_use['title_head'] = ucwords($_POST['title_head']);
        $data_use['title_content'] = ucfirst($_POST['title_content']);
        $data_use['url_content'] = $_POST['url_content'];
        $data_use['desc_content'] = ucfirst($_POST['desc_content']);
...
echo $txt;
  ...
  // QR-CODE
  if($page == 'qrcode'){
    $iframe = "../../theme/$tema/preview.php?user=$user&page=qrcode";
