...
function beginAnimation() {
    var el = document.querySelectorAll(".animate__animated");
    ...
    el.forEach((e) => {
        if (e.classList.contains("animate__fadeInDown")) {
            e.dataset["fadeInDown"] = null;
        } else if (e.classList.contains("animate__fadeInUp")) {
            e.dataset["fadeInUp"] = null;
        }
        if (e.classList.contains("animate__fadeInLeft")) {
            e.dataset["fadeInLeft"] = null;
        }
        if (e.classList.contains("animate__fadeInRight")) {
            e.dataset["fadeInRight"] = null;
        }
        if (e.classList.contains("animate__zoomIn")) {
            e.dataset["zoomIn"] = null;
        }

        e.classList.remove("animate__fadeInDown", "animate__fadeInUp", "animate__fadeInLeft", "animate__fadeInRight", "animate__zoomIn");
...
if (document.querySelector("#guestNameSlot, #guestNameSlot2")) {
    var guestNameSlot = document.querySelectorAll("#guestNameSlot, #guestNameSlot2");
    guestNameSlot.forEach((e) => (e.innerText = guestName));
}
...
openInvitation = (target) => {
    ...
    var fakeCover = document.querySelector("#cover.fake-cover");
    ...
    fakeCover.style.transform = "translateY(-100%)";
    if (window.innerWidth <= 500) setTimeout(() => document.querySelector(".invitation__list").scrollTo({ top: window.innerHeight, behavior: "instant" }), 100);

    // setTimeout(() => {
    //     document.querySelector(".invitation__list").scrollTo({ top: window.innerHeight, behavior: "smooth" });
    // }, 100);
...
var invToggle = (e) => {
    ...
    document.querySelectorAll(".btn-open-invitation").forEach((el) => (el.style.display = "none"));
    document.querySelector(".invitation__list").style.overflowY = "scroll";
};
...
var rsvpRadios = document.querySelectorAll(".rsvp-radio input");
var rsvpSelect = document.querySelector("#hadir");
rsvpRadios.forEach((e) => {
    e.addEventListener("change", (ev) => {
        if (ev.target.checked) rsvpSelect.value = e.value;
    });
});
