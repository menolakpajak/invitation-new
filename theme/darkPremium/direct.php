...
// $user = $_GET["user"] ?? null;
// AUTO INPUT
// echo isset($_GET['user']);

// if (!@$_GET["user"]) {
//     exit("hey <br/>" . json_encode($_GET));
// }
...
if (!isset($_COOKIE['user'])) {
    ...
    setcookie('user', $data['cookie'], time() + (2629800 * 6), '/;SameSite=None;Secure');
    setcookie('admin', $table, time() + (2629800 * 6), '/;SameSite=None;Secure');
    if (isset($kepada) && isset($kode)) {
        $kepada = urlencode($kepada);
        exit(header("Location: index.php?kepada=$kepada&kode=$kode"));
    }
